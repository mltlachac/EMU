...
def plot_pca_variance(var, num_f):
    ...
    plt.xticks(x_ticks)
    plt.savefig("machine_learning/experiments/feature_selection/" + args.modality + '_' + str(num_f) + "feature_var_PCA.png", bbox_inches = "tight")
    plt.close()
...
def plot_features(data, feature_name='n/a'):
    ...
    plt.scatter(depressed[0], depressed[1], alpha = 0.5, label = "Depressed", marker = 's', color = 'b')
    plt.scatter(not_depressed[0], not_depressed[1], alpha = 0.5, label = "Not Depressed", marker = '^', color = 'r')
    ...
    plt.xticks([], [])
    plt.yticks([], [])
    plt.savefig("machine_learning/experiments/feature_selection/" + args.modality + "_" + args.feature_selection + "_scatter.png", bbox_inches = "tight")
    plt.close()
...
def scale_feature_select(num_f, plot, cnames, random_state, data, label):
    ...
    if args.feature_selection == 'pca':
        ...
        pca_weights.to_csv('machine_learning/experiments/feature_selection/' + args.modality + '_' + str(num_f) + 'features_pca_weights.csv')
    ...
    if args.feature_selection == 'chi2' or args.feature_selection == 'etc':
        with open('machine_learning/experiments/feature_selection/' + args.modality + "_" + args.feature_selection + '_' + str(num_f) + 'features.json', 'w') as jf:
